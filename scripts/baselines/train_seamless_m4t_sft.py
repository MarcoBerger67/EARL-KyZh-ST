...
DEFAULT_TRAIN_FILE = "train_ky2zh_with_ky_text.jsonl"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="LoRA SFT for SeamlessM4T-v2 speech-to-text translation on compact ky->zh data."
    )
    parser.add_argument(
        "--base-model-path",
        type=str,
        default=str(root_dir / "model" / "seamless-m4t-v2-large"),
        help="Local SeamlessM4T-v2 model directory or Hugging Face model id.",
    )
    parser.add_argument(
        "--train-data-path",
        type=Path,
        default=root_dir / "data" / DEFAULT_TRAIN_FILE,
    ...
    parser.add_argument("--experiment-name", type=str, default="sft_seamless_m4t_v2_large_ky2zh_lora")
    parser.add_argument("--audio-prefix-from", type=str, default="")
    parser.add_argument("--audio-prefix-to", type=str, default="")
    parser.add_argument("--target-lang", type=str, default="cmn", help="Seamless target language code for Chinese.")
...
def load_audio(path: str, sampling_rate: int):
    try:
        ...
        raise RuntimeError("SeamlessM4T SFT requires librosa to load audio.") from exc
...
def encode_audio(processor: Any, audios: list[Any], sampling_rate: int) -> dict[str, torch.Tensor]:
    try:
        return processor(
            audio=audios,
...
def encode_targets(
    ...
    for kwargs in (
        {"text": targets, "tgt_lang": target_lang, **encode_kwargs},
        {"text": targets, "src_lang": target_lang, "tgt_lang": target_lang, **encode_kwargs},
        {"text": targets, "src_lang": target_lang, **encode_kwargs},
    ):
        try:
            return processor(**kwargs)["input_ids"]
        except TypeError:
            pass
    return tokenizer(targets, **encode_kwargs)["input_ids"]


@dataclass
class SeamlessSftCollator:
    processor: Any
    sampling_rate: int
    target_lang: str
    max_target_length: int
    ...
    def __call__(self, features: list[dict[str, str]]) -> dict[str, torch.Tensor]:
        ...
        inputs = encode_audio(self.processor, audios, self.sampling_rate)
        labels = encode_targets(
            self.processor,
            [row["reference"] for row in features],
            self.target_lang,
            self.max_target_length,
        )
        pad_token_id = getattr(getattr(self.processor, "tokenizer", None), "pad_token_id", 0)
...
def load_model_and_processor(args: argparse.Namespace):
    ...
    kwargs = get_model_kwargs(args)
    try:
        model = SeamlessM4Tv2ForSpeechToText.from_pretrained(args.base_model_path, **kwargs)
    except TypeError:
