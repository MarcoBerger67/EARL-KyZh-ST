...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate testt.jsonl with Whisper ASR + NLLB MT and testt.ner.jsonl entity metrics."
    )
    parser.add_argument(
        "--test-data-path",
        type=Path,
    ...
    parser.add_argument(
        "--whisper-model-path",
        type=str,
        default=str(root_dir / "model" / "whisper-large-v3"),
    )
    parser.add_argument(
        "--nllb-model-path",
        type=str,
        default=str(root_dir / "model" / "nllb-200-distilled-600M"),
    )
    parser.add_argument(
        "--nllb-adapter-path",
        type=str,
        default=None,
        help="Optional PEFT/LoRA adapter for the NLLB MT model.",
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=root_dir / "eval_outputs" / "whisper_nllb_testt",
    )
    ...
    parser.add_argument("--translation-max-new-tokens", type=int, default=None)
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Whisper language code. Kyrgyz/ky is not supported by Whisper; omit this or pass auto/none.",
    )
    parser.add_argument("--whisper-task", type=str, default="transcribe")
    parser.add_argument("--nllb-source-lang", type=str, default="kir_Cyrl")
    parser.add_argument("--nllb-target-lang", type=str, default="zho_Hans")
...
def build_eval_config(args: argparse.Namespace) -> dict[str, Any]:
    translation_max_new_tokens = args.translation_max_new_tokens or args.asr_max_new_tokens
    ...
    if isinstance(whisper_language, str) and whisper_language.strip().lower() in {"", "auto", "none", "null"}:
    ...
    return {
        ...
        "model": {
            ...
            "name": args.model_name or f"{Path(args.whisper_model_path).name}__{Path(args.nllb_model_path).name}",
            "base_model_path": args.whisper_model_path,
            ...
            "nllb_model_path": args.nllb_model_path,
            "nllb_adapter_path": args.nllb_adapter_path,
            "generation_kwargs": {"max_new_tokens": args.asr_max_new_tokens},
        },
        "evaluation": {
...
def main() -> None:
    ...
    config_path = args.config_out or (args.output_dir / "whisper_nllb_testt.resolved.yaml")
