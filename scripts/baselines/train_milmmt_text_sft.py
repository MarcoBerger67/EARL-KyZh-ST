...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="LoRA SFT for MiLMMT/Gemma3 text MT on compact Kyrgyz text -> Chinese text data."
    )
    parser.add_argument(
        "--base-model-path",
        type=str,
        default=str(root_dir / "model" / "MiLMMT-46-4B-v0.1"),
        help="Local MiLMMT model directory or Hugging Face model id.",
    )
    parser.add_argument(
        "--train-data-path",
        type=Path,
    ...
    parser.add_argument("--experiment-name", type=str, default="sft_milmmt_46_4b_ky2zh_lora")
...
def load_rows(args: argparse.Namespace) -> list[dict[str, str]]:
    ...
    with args.train_data_path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            target_text = normalize_text(record.get("text_zh", ""))
...
def build_prompt(row: dict[str, str], template: str) -> str:
    return template.format(source_text=row["source_text"])


def build_prompt_messages(row: dict[str, str], template: str) -> list[dict[str, str]]:
    return [{"role": "user", "content": build_prompt(row, template)}]


def build_training_messages(row: dict[str, str], template: str) -> list[dict[str, str]]:
    return build_prompt_messages(row, template) + [{"role": "assistant", "content": row["text_zh"]}]


def _as_input_ids(value: Any) -> list[int]:
    ...
    def __call__(self, features: list[dict[str, str]]) -> dict[str, torch.Tensor]:
        ...
        for row in features:
            prompt_ids = encode_chat(
                self.tokenizer,
                build_prompt_messages(row, self.prompt_template),
                add_generation_prompt=True,
            )
            full_ids = encode_chat(
                self.tokenizer,
                build_training_messages(row, self.prompt_template),
        ...
        attention_mask = []
        labels = []
        for ids, row_labels in zip(batch_input_ids, batch_labels):
...
def main() -> None:
    ...
    run_config = {
        ...
        "task": "kyrgyz_text_to_chinese_mt",
