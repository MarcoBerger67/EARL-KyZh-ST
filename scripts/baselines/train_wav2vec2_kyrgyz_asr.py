...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "LoRA/domain SFT for Kyrgyz Wav2Vec2-XLSR CTC ASR. "
            "This trains audio -> source_text, not speech -> Chinese translation."
        )
    )
    parser.add_argument(
        "--base-model-path",
        type=str,
        default=str(root_dir / "model" / "wav2vec2-large-xlsr-53-kyrgyz"),
        help="Local Wav2Vec2 Kyrgyz ASR model directory or Hugging Face model id.",
    )
    parser.add_argument(
        "--train-data-path",
        type=Path,
    ...
    parser.add_argument("--experiment-name", type=str, default="sft_wav2vec2_xlsr53_kyrgyz_asr_lora")
    parser.add_argument("--audio-prefix-from", type=str, default="")
    parser.add_argument("--audio-prefix-to", type=str, default="")
    parser.add_argument(
        "--target-field",
        choices=["source_text", "text_zh"],
        default="source_text",
        help="Use source_text for Kyrgyz ASR. text_zh is only for experiments and is not recommended.",
    )
...
def load_audio(path: str, sampling_rate: int):
    try:
        ...
        raise RuntimeError("Wav2Vec2 ASR training requires librosa to load audio.") from exc
            ...
            rows.append(
                {
                    ...
                    "text_zh": normalize_text(record.get("text_zh", "")),
                }
            )
    if not rows:
...
class Wav2Vec2CtcCollator:
    ...
    def __call__(self, features: list[dict[str, str]]) -> dict[str, torch.Tensor]:
        ...
        targets = [row["target"] for row in features]
        try:
        ...
        inputs["labels"] = labels.masked_fill(labels == pad_token_id, -100)
        return inputs
...
def load_model_and_processor(args: argparse.Namespace):
    ...
    try:
        model = AutoModelForCTC.from_pretrained(args.base_model_path, **kwargs)
    except TypeError:
...
def main() -> None:
    ...
    if args.target_field != "source_text":
        print(json.dumps({"warning": "Wav2Vec2 CTC is ASR; source_text is the recommended target."}, ensure_ascii=False))
