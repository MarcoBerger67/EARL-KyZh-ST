...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate testt.jsonl with Qwen3-ASR + NLLB cascade and entity metrics."
    )
    parser.add_argument(
        "--test-data-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=str,
        default=str(root_dir / "model" / "Qwen3-ASR-1.7B"),
    )
    parser.add_argument(
        "--nllb-model-path",
        type=str,
        default=str(root_dir / "model" / "nllb-200-3.3B"),
    )
    parser.add_argument("--nllb-adapter-path", type=str, default="")
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=root_dir / "eval_outputs" / "qwen3_asr_nllb_testt",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="eager",
        help="Attention implementation passed to Qwen3-ASR. Default eager avoids SDPA mask shape incompatibilities.",
    )
    parser.add_argument(
        "--asr-language",
        type=str,
        default=None,
        help="Optional Qwen3-ASR language hint. Leave unset for auto language detection.",
    )
...
def build_eval_config(args: argparse.Namespace) -> dict[str, Any]:
    return {
        ...
        "model": {
            "kind": "qwen3_asr_nllb_cascade",
            "name": args.model_name or "qwen3-asr-1.7b-sft__nllb-200-3.3b-sft",
            "base_model_path": args.qwen3_asr_model_path,
            ...
            "asr_batch_size": args.asr_batch_size or args.batch_size,
            "asr_attn_implementation": args.asr_attn_implementation,
        },
        "evaluation": {
...
def main() -> None:
    ...
    config_path = args.config_out or (args.output_dir / "qwen3_asr_nllb_testt.resolved.yaml")
