...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate testt.jsonl with Wav2Vec2 CTC ASR + NLLB MT and aligned metrics."
    )
    parser.add_argument(
        "--test-data-path",
        type=Path,
    ...
    parser.add_argument(
        "--wav2vec2-adapter-path",
        type=str,
        default="",
        help="Optional Wav2Vec2 ASR LoRA adapter path. Empty means evaluate the base ASR model.",
    )
    parser.add_argument(
        "--nllb-model-path",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=root_dir / "eval_outputs" / "wav2vec2_xlsr53_kyrgyz_asr_nllb_3p3b_testt_lcs",
    )
...
def build_eval_config(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "model": {
            "kind": "wav2vec2_ctc_nllb_cascade",
            "name": args.model_name or "wav2vec2-xlsr53-kyrgyz-asr-lora+nllb-3p3b-sft",
            "base_model_path": args.wav2vec2_model_path,
            "adapter_path": args.wav2vec2_adapter_path,
...
def main() -> None:
    ...
    config_path = args.config_out or (args.output_dir / "wav2vec2_nllb_testt.resolved.yaml")
