...
DEFAULT_LORA_TARGET_MODULES = "q,k,v,o,wi,wi_0,wi_1,wo"


def parse_args() -> argparse.Namespace:
    root_dir = Path(__file__).resolve().parents[2]
    parser = argparse.ArgumentParser(
        description="LoRA SFT for MADLAD400 MT on compact Kyrgyz text -> Chinese text data."
    )
    parser.add_argument(
        "--base-model-path",
        type=str,
        default=str(root_dir / "model" / "madlad400-3b-mt"),
        help="Local MADLAD400 model directory or Hugging Face model id.",
    )
    parser.add_argument(
        "--train-data-path",
        type=Path,
    ...
    parser.add_argument("--experiment-name", type=str, default="sft_madlad400_3b_mt_ky2zh_lora")
    parser.add_argument(
        "--target-prefix",
        type=str,
        default="<2zh>",
        help='MADLAD target language prefix prepended to source text, e.g. "<2zh>".',
    )
    parser.add_argument("--source-field", choices=["source_text", "prompt_plus_source_text"], default="source_text")
...
def build_source(row: dict[str, str], mode: str, target_prefix: str) -> str:
    if mode == "prompt_plus_source_text":
        text = f"{row['prompt']}\n\n{row['source_text']}"
    else:
        text = row["source_text"]
    return f"{target_prefix} {text}".strip()
...
class MadladMtCollator:
    ...
    def __call__(self, features: list[dict[str, str]]) -> dict[str, torch.Tensor]:
        sources = [build_source(row, self.source_field, self.target_prefix) for row in features]
        targets = [row["text_zh"] for row in features]
